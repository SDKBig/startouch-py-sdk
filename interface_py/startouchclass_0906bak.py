...
class SingleArm:
    ...
    def gravity_compensation(self) -> bool:
        self.arm.gravity_compensation()  #另起一个线程 会随着类的结束而结束
        return True

    def set_joint(
        self,
        positions: Union[List[float], np.ndarray],  # Shape: (num_joints,)
    ...
    def set_joint_posvel(
        ...
        """
        self.arm.set_joint_posvel(q_end = positions,tf = tf, ctrl_hz = ctrl_hz)
    ...
    def switch_motors_mode(self, mode: int = 1):
        """
        Switch the control mode of the motors.

        Args:
            mode: Control mode to switch to. Default is 1.
            // MIT_MODE       = 1,
            // POS_VEL_MODE   = 2,
            // VEL_MODE       = 3,暂未实现
            // POS_FORCE_MODE = 4，暂未实现
        """
        self.arm.switch_motors_mode(mode)
    ...
    def set_end_effector_pose_euler_posvel(
        self,
        pos: Optional[Union[List[float], np.ndarray]] = None,  # Shape: (3,)
        euler: Optional[Union[List[float], np.ndarray]] = None,  # Shape: (3,)
        tf: float = 4.0,  # 默认时间：4.0秒 但位置速度控制模式和linear插值模式下不生效
    ) -> bool:
        """
        /**
        * @brief 设置末端执行器的目标位姿
        ...
        */
        """
        self.arm.set_end_effector_pose_posvel(target_pos=pos, target_euler=euler, tf=tf)
